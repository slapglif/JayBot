...
def run(data,tp=35,wait=1,slip=5):
...
def main():
 ...
 s,t,e,p=run(data);out=Path('backtest_results/kitty_2_5pct_tp35_2y_hourly');out.mkdir(parents=True,exist_ok=True);t.assign(order_date_et=pd.to_datetime(t.timestamp,utc=True).dt.tz_convert('US/Eastern').dt.strftime('%Y-%m-%d')).to_csv(out/'orders_with_dates.csv',index=False);e.to_csv(out/'equity_curve.csv');(out/'summary.json').write_text(json.dumps({'stats':s,'inceptions':inc},indent=2));print(json.dumps(s,indent=2));print('inceptions',inc)
