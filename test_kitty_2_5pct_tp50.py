...
def run(data,tp=50,wait=1,slip=5):
 ...
 for dt in decisions:
  ...
  for drop,s,px in sorted(elig):
   ...
   order=equity*.025;fill=px*(1+slip/10000);qty=order/fill
 ...
 stats={'start':sessions[0],'end':sessions[-1],'sessions':len(sessions),'final':final,'return_pct':(final/100000-1)*100,'cagr_pct':((final/100000)**(1/years)-1)*100,'max_dd_pct':dd*100,'orders':len(td),'buys':int((td.side=='BUY').sum()),'exits':int((td.side=='EXIT').sum()),'open':len(pos),'tp':tp,'wait':wait,'purchases_per_day':2,'entry_pct':2.5,'max_asset_pct':10}
 return stats,td,eq,pos
def main():
 data={};inc={}
 for e,u in PAIRS.items():data[e],f=stitch(e,u);inc[e]=str(f.date()) if f is not None else None
 s,t,e,p=run(data);out=Path('backtest_results/kitty_2_5pct_tp50_2y_hourly');out.mkdir(parents=True,exist_ok=True);t.assign(order_date_et=pd.to_datetime(t.timestamp,utc=True).dt.tz_convert('US/Eastern').dt.strftime('%Y-%m-%d')).to_csv(out/'orders_with_dates.csv',index=False);e.to_csv(out/'equity_curve.csv');(out/'summary.json').write_text(json.dumps({'stats':s,'inceptions':inc},indent=2));print(json.dumps(s,indent=2));print('inceptions',inc)
