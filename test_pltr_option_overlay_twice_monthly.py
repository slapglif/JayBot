...
def main():
 ...
 out=Path('backtest_results/pltr_option_overlay_twice_monthly');out.mkdir(parents=True,exist_ok=True);res=pd.DataFrame(rows,columns=['strike_offset_dollars','option_profit','wins','losses','skipped_budget','executed_trades','win_rate_pct']).sort_values('option_profit',ascending=False);res.to_csv(out/'strike_3_to_8_results.csv',index=False)
 for k,v in ledgers.items():v.to_csv(out/f'offset_{k}_ledger.csv',index=False)
 (out/'summary.json').write_text(json.dumps({'method':'Synthetic Black-Scholes call overlay; first two PLTR strategy buys per month; 28 calendar DTE; 20-day realized vol clipped 35%-150%; integer contracts; premium <=0.2% equity; exit +50% or expiry.','results':res.to_dict('records')},indent=2));print(res.to_string(index=False))
