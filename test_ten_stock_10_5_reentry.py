...
def run(data,tp=29,wait=1,slip=5):
 ...
 for dt in bars:
  ...
  for s,p in list(pos.items()):
   ...
   r=data[s].loc[dt];avg=p.cost/p.qty;target=avg*(1+tp/100);stop=avg*1.05
   if r.High>=target:
   ...
   if not p.armed and r.High>=avg*1.10:p.armed=True;arms.append([dt,s,avg,stop])
   if p.armed and r.Low<=stop:
    fill=stop*(1-slip/10000);pro=p.qty*fill;cash+=pro;tr.append([dt,s,'EXIT',p.qty,fill,pro-p.cost,p.entries,'PROFIT_LOCK_5']);del pos[s];closed.append(s)
 ...
 stats={'start':sessions[0],'end':sessions[-1],'final':final,'return_pct':(final/100000-1)*100,'cagr_pct':((final/100000)**(1/years)-1)*100,'max_dd_pct':dd*100,'orders':int((td.side!='NO_BUY').sum()),'buys':int((td.side=='BUY').sum()),'exits':int((td.side=='EXIT').sum()),'tp_exits':int((td.reason=='TP29').sum()),'profit_lock_exits':int((td.reason=='PROFIT_LOCK_5').sum()),'post_exit_buys':int((td.reason=='POST_EXIT_2H').sum()),'post_exit_no_qualifier':int((td.reason=='POST_EXIT_NO_QUALIFIER').sum()),'open':len(pos),'arms':len(ar)}
 return stats,td,eq,ar
if __name__=='__main__':
 data={e:stitch(e,u)[0] for e,u in PAIRS.items()};s,t,e,a=run(data);out=Path('backtest_results/ten_stock_10_5_reentry');out.mkdir(parents=True,exist_ok=True);t.to_csv(out/'orders_and_checks.csv',index=False);e.to_csv(out/'equity.csv');a.to_csv(out/'stop_activations.csv',index=False);(out/'summary.json').write_text(json.dumps(s,indent=2));print(json.dumps(s,indent=2));print(t.reason.value_counts().to_string())
